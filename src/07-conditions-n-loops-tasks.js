...
function doRectanglesOverlap(rect1, rect2) {
  return rect1.width + rect1.left > rect2.left && rect1.height + rect1.top > rect2.top;
}
...
function getDigitalRoot(num) {
  const sum = String(num).split('').map((el) => Number(el)).reduce((acc, currV) => acc + currV);
  if (sum > 9) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    if (open.includes(arr[i])) {
      result.push(arr[i]);
    } else if (brackets[result.pop()] !== arr[i]) {
      return false;
    }
  }
...
function getCommonDirectoryPath(pathes) {
  let result = '';
  const newArr = pathes.map((el) => el.split('/'));
...
function evaluateTicTacToePosition(position) {
  ...
  //     if (position[i].every((el) => el === values[j])) {
  //       check = values[j];
  //     }
  //     if (check) return;
  //   }
